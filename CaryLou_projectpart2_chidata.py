...
def main(argv):
    explorechidata('ChicagoCrime2016W3newfeatures.tsv','chidata2016cleaned.tsv','dataexploration.txt','ChicagoDataCheckVals.csv')    
    plotchidata('chidata2016cleaned.tsv', 'plotoutcomes.txt')
    clusterchidata('chidata2016cleaned.tsv','clusteroutcomes.txt')
    assocruleschidata('chidata2016cleaned.tsv','assocoutcomes.txt')
    predictchidata('chidata2016cleaned.tsv','predictoutcomes.txt')
...
def clusterchidata(data, out):
    ...
    #with open("chidata2016cleaned.tsv", 'r') as file:
        ...
        with open(out, 'w') as outfile:
            ...
            plt.savefig('kmeans3latlong.png')
            plt.clf()
            plt.close()    
...
#            plt.show() 
    ...
    #Other Algorithms
            ...
            # Plot using a scatter plot and shade by cluster label
            plt.scatter(x=plot_columns[:,0], y=plot_columns[:,1], c=labels)
            plt.savefig('dbscanpca2.png')
            plt.clf()
            plt.close()     
            
            # Plot using a scatter plot and shade by cluster label geographically
            plt.scatter(x=myDataNoNA['longitude'], y=myDataNoNA['latitude'], c=labels, s= 1, lw=0)
            plt.savefig('dbscanlatlong.png')
            plt.clf()
            plt.close()    
            ...
            # Plot using a scatter plot and shade by cluster label
            plt.scatter(x=plot_columns[:,0], y=plot_columns[:,1], c=labels)
            plt.savefig('wardpca2.png')
            plt.clf()
            plt.close()          
            
            # Plot using a scatter plot and shade by cluster label geographically
            plt.scatter(x=myDataNoNA['longitude'], y=myDataNoNA['latitude'], c=labels, s= 1, lw=0)
            plt.savefig('wardlatlong.png')
            plt.clf()
            plt.close()  
